...
class CaptureController : NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
	...
	var recycledPreviewLayers: [AVCaptureVideoPreviewLayer] = [] // We recycle preview layers because we crash otherwise on 10.9
	...
	public func register(_ videoLayer: VideoInputLayer) {
		...
		videoLayer.previewLayer = recycledPreviewLayers.popLast() ?? AVCaptureVideoPreviewLayer()
		captureQueue.async {
			videoLayer.previewLayer!.session = self.captureSession
			
			DispatchQueue.main.async {
				videoLayer.previewLayer!.connection?.videoOrientation = self.videoOrientation
				// Mirror video if appropriate and not already mirrored
				videoLayer.isMirrored = (self.captureDevice?.position != .back && !(videoLayer.previewLayer!.connection?.isVideoMirrored ?? false))
			}
		}
	...
	public func teardown(_ videoLayer: VideoInputLayer) {
		assertOnQueue(.main)
		if let previewLayer = videoLayer.previewLayer {
			recycledPreviewLayers.append(previewLayer)
			videoLayer.previewLayer = nil
			updatePreviewing()
		}
	}
	...
	func reconfigureCaptureOrientation() {
		...
		DispatchQueue.main.async { [videoOrientation] in
			self.videoLayers.allObjects.forEach { videoLayer in
				videoLayer.previewLayer?.connection?.videoOrientation = videoOrientation
				// Mirror video if appropriate and not already mirrored
				videoLayer.isMirrored = (self.captureDevice?.position != .back && !(videoLayer.previewLayer!.connection?.isVideoMirrored ?? false))
			}
		}
	...
	func reconfigureCaptureFormat() {
		...
		do {
			...
			if let biggestFormat = biggestFormat {
				if captureDevice.activeFormat != biggestFormat {
	...
	@objc var profile: VideoProfile = .h264Baseline {
		didSet {
			if profile != oldValue {
				encoderSettingsInvalidated = true
			}
	...
	@objc var targetBitRate: Int = 0 {
		didSet {
			if targetBitRate != oldValue {
				encoderSettingsInvalidated = true
			}
		}
	}
	@objc var maxPacketSize: Int = 0 {
		didSet {
			if maxPacketSize != oldValue {
				encoderSettingsInvalidated = true
			}
		}
	}
	@objc var intraFrameRate: Int = Int.max {
		didSet {
			if intraFrameRate != oldValue {
				encoderSettingsInvalidated = true
			}
		}
	}
	...
	@objc public var forceSoftwareEncoding = false
	...
	func createEncoder() {
		...
		// Try to use hardware encoding first
		if AppleEncoder.supportedCodecs.contains(videoCodec) && !forceSoftwareEncoding {
			do {
				videoEncoder = try AppleEncoder(codec: videoCodec, targetDimensions: targetDimensions, outputCallback: callback)
			} catch {
				print("SCICaptureController: Failed to create encoder: \(error)")
			}
		}
	...
	func captureOutput(_ captureOutput: AVCaptureOutput,
		...
		do {
			try videoEncoder.encode(sampleBuffer, forceKeyframe: requestedKeyframe)
			requestedKeyframeLock.lock()
	...
	@objc public var availableCodecs: [CMVideoCodecType] {
		...
		// Return a list of codecs supported by all the encoders, removing duplicates
		return Array(Set([
			AppleEncoder.supportedCodecs
		].flatMap { $0 }))
	}
