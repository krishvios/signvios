...
void CropScaleBin::create()
{
	...
	GStreamerElement captureImxPTZ;
	...
	stiTESTCOND(imxConvertFactory.get (), stiRESULT_ERROR);
	...
	captureImxPTZ = imxConvertFactory.createElement(PTZ_IMX_ELEMENT_NAME);
	stiTESTCOND(captureImxPTZ.get (), stiRESULT_ERROR);

	captureImxPTZ.propertySet ("videocrop-meta-enable", true);
	...
	if (m_enableConvert)
	{
		// this is primarily to support jpeg-encoder which doesn't take RGBA
	...
	elementAdd (captureImxPTZ);
	elementAdd (ptzScaleFilter);

	result = capturePTZ.link (captureImxPTZ);
	stiTESTCOND(result, stiRESULT_ERROR);

	result = captureImxPTZ.link (ptzScaleFilter);
	stiTESTCOND(result, stiRESULT_ERROR);
	...
	const CstiCameraControl::SstiPtzSettings &ptzRectangle, int frameWidthIn, int frameHeightIn)
{
	...
	if (!empty())
	{
		...
		if (m_enableScale && frameWidthIn > 0 && frameHeightIn > 0)
		{
			...
			std::string capsStr = "video/x-raw,format=RGBx,width=";
